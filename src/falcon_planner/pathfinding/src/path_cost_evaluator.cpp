...
double PathCostEvaluator::searchPath(const Vector3d &p1, const Vector3d &p2, vector<Vector3d> &path) {
  ...
  while (caster_->nextId(idx)) {
    if (map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::OCCUPIED ||
        map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::UNKNOWN || !map_server_->getTSDF()->isInBox(idx)) {
      safe = false;
      break;
    }
  }
...
double PathCostEvaluator::searchPathBBox(const Vector3d &p1, const Vector3d &p2, const Vector3d &bbox_min,
  ...
  while (caster_->nextId(idx)) {
    if (map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::OCCUPIED ||
        map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::UNKNOWN ||
...
double PathCostEvaluator::searchPathUnknown(const Vector3d &p1, const Vector3d &p2, vector<Vector3d> &path) {
  ...
  caster_->input(p1, p2);
  while (caster_->nextId(idx)) {
    if (map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::OCCUPIED || !map_server_->getTSDF()->isInBox(idx)) {
      safe = false;
      break;
    }
  }
...
double PathCostEvaluator::searchPathUnknownBBox(const Vector3d &p1, const Vector3d &p2,
  ...
  while (caster_->nextId(idx)) {
    ...
    if (map_server_->getOccupancyGrid()->getVoxel(idx).value == voxel_mapping::OccupancyType::OCCUPIED || !isInBox(p)) {
      safe = false;
      break;
    }
  }
...
double PathCostEvaluator::computeCost(const Vector3d &p1, const Vector3d &p2, const double &y1, const double &y2, 
                                      const Vector3d &v1, const double &yd1, vector<Vector3d> &path, bool verbose) {
