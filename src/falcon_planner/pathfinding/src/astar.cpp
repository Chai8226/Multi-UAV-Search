...
int Astar::search(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &end_pt) {
  ...
  while (!open_set_.empty()) {
    ...
    if (true) {
      ...
      for (double l = 0.1; l < len; l += 0.1) {
        Vector3d ckpt = cur_node->position + l * dir;
        if (map_server_->getOccupancyGrid()->getVoxel(ckpt).value == voxel_mapping::OccupancyType::OCCUPIED ||
            map_server_->getOccupancyGrid()->getVoxel(ckpt).value == voxel_mapping::OccupancyType::UNKNOWN) {
          safe = false;
          break;
        }
      }
    ...
    for (double dx = -config_.resolution_ + config_.epsilon_; dx <= config_.resolution_ + config_.epsilon_; dx += config_.resolution_)
      for (double dy = -config_.resolution_ + config_.epsilon_; dy <= config_.resolution_ + config_.epsilon_; dy += config_.resolution_)
        for (double dz = -config_.resolution_ + config_.epsilon_; dz <= config_.resolution_ + config_.epsilon_; dz += config_.resolution_) {
          step << dx, dy, dz;
          if (step.norm() < config_.epsilon_)
            continue;
          nbr_pos = cur_pos + step;
          ...
          if (map_server_->getOccupancyGrid()->getVoxel(nbr_pos).value == voxel_mapping::OccupancyType::OCCUPIED ||
              map_server_->getOccupancyGrid()->getVoxel(nbr_pos).value == voxel_mapping::OccupancyType::UNKNOWN) {
            continue;
          }
          ...
          if (node_iter == open_set_map_.end()) {
            ...
            use_node_num_ += 1;
            if (use_node_num_ == config_.allocate_num_) {
              ROS_WARN("[AStar] Run out of node pool. Duration: %f", (ros::Time::now() - t1).toSec());
              return NO_PATH;
            }
            neighbor->index = nbr_idx;
            neighbor->position = nbr_pos;
...
int Astar::searchUnknownBBox(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &end_pt,
  ...
  while (!open_set_.empty()) {
    ...
    bool reach_end = cur_node->index(0) == end_index(0) && cur_node->index(1) == end_index(1) && cur_node->index(2) == end_index(2);
