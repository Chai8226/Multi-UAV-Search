...
Transformer::Transformer(ros::NodeHandle &nh) : world_frame_("world"), sensor_frame_("camera") {
  ...
  case Config::POSE_TYPE::POSE:
    transform_sub_ = nh.subscribe("sensor_pose_topic", 100, &Transformer::poseCallback, this);
    break;
  case Config::POSE_TYPE::ODOM:
    transform_sub_ = nh.subscribe("sensor_pose_topic", 100, &Transformer::odometryCallback, this);
    break;
  case Config::POSE_TYPE::TRANSFORM:
    transform_sub_ = nh.subscribe("sensor_pose_topic", 100, &Transformer::transformCallback, this);
    break;
...
void Transformer::poseCallback(const PoseStampedMsgType &msg) {
  ...
  std::cout << "pose: " << msg.pose.position.x << ", " << msg.pose.position.y << ", " << msg.pose.position.z
