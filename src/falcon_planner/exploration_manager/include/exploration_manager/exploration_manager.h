...
public:
  ...
  int planExploreMotionHGrid(const Vector3d &pos, const Vector3d &vel, const Vector3d &acc, const Vector3d &yaw);
  EXPL_RESULT planTrajToView(const Vector3d &pos, const Vector3d &vel, const Vector3d &acc, const Vector3d &yaw, const Vector3d &next_pos, const double &next_yaw);
...
private:
  ...
  template <typename T>
  void saveCostMatrix(const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> &mat, const string &filename);
