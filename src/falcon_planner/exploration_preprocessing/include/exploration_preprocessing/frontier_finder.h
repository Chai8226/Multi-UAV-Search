...
public:
  ...
  void computeFrontiersToVisitNew(const vector<Vector3d> & all_centers_unknown_active);
...
private:
  ...
  void sampleViewpointsNew(Frontier &frontier, const vector<Vector3d> & all_centers_unknown_active);
  ...
  double candidate_rmax_, candidate_rmin_, candidate_dphi_, min_candidate_dist_, min_candidate_clearance_, min_candidate_occupied_clearance_;
  int down_sample_;
