...
struct ConnectivityNode {
  ...
  ConnectivityNode() {}
  ConnectivityNode(const int &id, const Position &pos, const std::pair<Position, Position> &bbox, const TYPE &type) : id_(id), pos_(pos), bbox_(bbox), type_(type) {}
  ~ConnectivityNode() {}
  ...
  void addNeighborWithPath(const int &neighbor_id, const double &cost, const ConnectivityEdge::TYPE &type, const std::vector<Position> &path) {
    ConnectivityEdge edge;
    edge.id1_ = id_;
    edge.id2_ = neighbor_id;
    edge.cost_ = cost;
  ...
  std::pair<Position, Position> bbox_;
...
public:
  ...
  void getFreeNodePositions(std::vector<Position> &node_positions);
  void getUnknownNodePositions(std::vector<Position> &node_positions);
  ...
  void getFullConnectivityGraph(std::vector<std::pair<Position, Position>> &edges, std::vector<ConnectivityEdge::TYPE> &edges_types, std::vector<double> &edges_costs);
  void getFullConnectivityGraphPath(std::vector<std::vector<Position>> &paths, std::vector<ConnectivityEdge::TYPE> &edges_types);
