...
struct GridCell {
  ...
  // Swarm
  std::vector<std::pair<Position, Position>> bboxes_free_, bboxes_unknown_;
...
public:
  ...
  void getGridCellIdsInBBox(const Position &bbox_min, const Position &bbox_max, std::vector<int> &cell_ids);

  // Grid update operations
  // Update frontier information in each cell
  void inputFrontiers(const std::vector<Position> &frontier_viewpoints, const std::vector<double> &frontier_yaws);
  ...
  Position rectifyCellCenter(const int &cell_id, const Position &center, const Position &bbox_min, const Position &bbox_max);
  Position rectifyCellCenter2(const int &cell_id, const Position &center, const Position &bbox_min, const Position &bbox_max);
  void getCCLCenters(const int &cell_id, std::vector<Position> &centers_free, std::vector<Position> &centers_unknown);
  void getCCLCenters2D(const int &cell_id, std::vector<Position> &centers_free, std::vector<Position> &centers_unknown);
  void getCCLCenters2DNew(const int &cell_id, std::vector<Position> &centers_free, std::vector<Position> &centers_unknown,
  ...
  double getAStarCostUnknown(const Position &pos1, const Position &pos2, const Eigen::Vector3d &vel);
  double getAStarCostYaw(const Position &pos1, const Position &pos2, const Eigen::Vector3d &vel, const double &yaw1, const double &yaw2);
  double getAStarCostBBox(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max);
  double getAStarCostBBox(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max, std::vector<Position> &path);
  double getAStarCostBBoxUnknown(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max);
  double getAStarCostBBoxUnknown(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max, std::vector<Position> &path);
  double getAStarCostBBoxUnknownOnly(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max);
  double getAStarCostBBoxUnknownOnly(const Position &pos1, const Position &pos2, const Position &bbox_min, const Position &bbox_max, std::vector<Position> &path);
  double getAStarCostHGridGraph(const Position &pos1, const Position &pos2);
  void calculateCostMatrixSingleThread(const Position &cur_pos, const Eigen::Vector3d &cur_vel, Eigen::MatrixXd &cost_matrix, std::map<int, std::pair<int, int>> &cost_mat_id_to_cell_center_id);
  void calculateCostMatrixSingleThreadSwarm(const Position &cur_pos, const Eigen::Vector3d &cur_vel, 
                                            const vector<std::pair<int, int>> &last_cell_center_id, Eigen::MatrixXd &cost_matrix, std::map<int, 
                                            std::pair<int, int>> &cost_mat_id_to_cell_center_id);
  void calculateCostMatrixMultiThread(const Position &cur_pos, const Eigen::Vector3d &cur_vel, const int &thread_num, Eigen::MatrixXd &cost_matrix, std::map<int, std::pair<int, int>> &cost_mat_id_to_cell_center_id);
  ...
  Position getCellSize() { return config_.cell_size_; }
  ...
  void getCellCentersWithType(const int &cell_id, const int &center_id, Position &center, CENTER_TYPE &type) {
    const GridCell &cell = uniform_grid_[cell_id];
    if (center_id < cell.centers_free_active_.size()) {
      center = cell.centers_free_active_[center_id];
      type = CENTER_TYPE::FREE;
  ...
  void getCellFrontiers(const int &id, std::vector<int> &frontier_ids, std::vector<Position> &frontier_vps) {
    frontier_ids = uniform_grid_[id].frontier_ids_;
    frontier_vps = uniform_grid_[id].frontier_viewpoints_;
  }
  void getCellCenterFrontiers(const int &cell_id, const int &center_id, std::vector<int> &frontier_ids, std::vector<Position> &frontier_vps) {
  ...
  void getVisualizationLineLists2D(std::vector<Position> &list1, std::vector<Position> &list2, double height = 1.0);
  void getVisualizationConnectivityLineLists(std::vector<Position> &list1, std::vector<Position> &list2);
  void getVisualizationIdText(std::vector<std::string> &cell_ids, std::vector<Position> &cell_centers, std::vector<std::vector<Position>> &cell_centers_free, std::vector<std::vector<Position>> &cell_centers_unknown);
  
  // sawrm
  // 1 - free, 0 - unknown
  void getCellBboxWithID(const int &id, std::vector<std::pair<Position, Position>> &free_bboxes, 
                         std::vector<std::pair<Position, Position>> &unknown_bboxes) {
    free_bboxes = uniform_grid_[id].bboxes_free_;
    unknown_bboxes = uniform_grid_[id].bboxes_unknown_;
    return;
  }
...
public:
  ...
  // swarm
  std::vector<UniformGrid> swarm_uniform_grids_;
  ...
  HierarchicalGrid(ros::NodeHandle &nh, voxel_mapping::MapServer::Ptr &map_server, int &drone_id);

  // Update grid information from frontier
  void inputFrontiers(const std::vector<Position> &frontier_viewpoints, const std::vector<double> &frontier_yaws);
  void updateHierarchicalGridFromVoxelMap(const Position &update_bbox_min,const Position &update_bbox_max);
  void updateHierarchicalGridFrontierInfo(const Position &update_bbox_min, const Position &update_bbox_max);

  // Cost matrix calculation for global planning
  void calculateCostMatrix2(const Position &cur_pos, const Eigen::Vector3d &cur_vel, const double &cur_yaw, const std::vector<Position> &last_path,
  ...
  void getLayerCellCenters(const int &level, const int &cell_id, const int &center_id, Position &center) {
    uniform_grids_[level].getCellCenters(cell_id, center_id, center);
  }
  void getLayerCellCenterType(const int &level, const int &cell_id, const int &center_id, UniformGrid::CENTER_TYPE &center_type) {
  ...
  void getLayerPositionCellCenterId(const int &level, const Position &pos, int &cell_id, int &center_id) {
  ...
  void publishGridsLayerNew(const int &level = 0);
  ...
  // swarm
  void getUniformLayerCellIds(const int &level, std::vector<int> &cell_ids) {
    cell_ids.clear();
    auto &u_gs = uniform_grids_[level];
    for (int i = 0; i < u_gs.uniform_grid_.size(); ++i) {
      cell_ids.push_back(u_gs.uniform_grid_[i].id_);
    }
  }
  void getSwarmLayerCellIds(const int &level, std::vector<int> &cell_ids) {
    cell_ids.clear();
    auto &u_gs = swarm_uniform_grids_[level];
    for (int i = 0; i < u_gs.uniform_grid_.size(); ++i) {
      cell_ids.push_back(u_gs.uniform_grid_[i].id_);
    }
  }
  void updateSwarmUniformGrid(const int &level, std::vector<int> &new_cell_id) {
    auto ug = uniform_grids_[level].uniform_grid_;
    swarm_uniform_grids_[level].uniform_grid_.clear();
    for (auto &id : new_cell_id) {
      auto it = std::find_if(ug.begin(), ug.end(), [&id](const GridCell &cell) { return cell.id_ == id; });
      if (it != ug.end()) {
        swarm_uniform_grids_[level].uniform_grid_.push_back(*it);
      }
    }
  }
  bool idInSwarmUniformGrid(const int &id) {
    for (auto &ug : swarm_uniform_grids_) {
      auto it = std::find_if(ug.uniform_grid_.begin(), ug.uniform_grid_.end(), [&id](const GridCell &cell) { return cell.id_ == id; });
      if (it != ug.uniform_grid_.end()) {
        return true;
      }
    }
    return false;
  }
  void calculateCostMatrixForSwarm(const Position &cur_pos, const Eigen::Vector3d &cur_vel, const double &cur_yaw,
  ...
  void calculateCostMatrixForSwarmOpt(const Position &pos1, const Position &pos2,
                                     ...
                                     std::map<int, std::pair<int, int>> &cost_mat_id_to_cell_center_id);                                  
  void classifyUniformGrids(const int& level, std::vector<int>& free_ids, std::vector<int>& unknown_ids);
  void classifyUniformGrids2(const int& level, const std::vector<int>& total_ids, std::vector<int>& free_ids, std::vector<int>& unknown_ids);
  bool areDronesConnected(const int& level, const std::vector<int>& ids1, const std::vector<int>& ids2, const std::vector<int>& dormant_ids);
