...
FrontierFinder::FrontierFinder(ros::NodeHandle &nh, const shared_ptr<MapServer> &map_server) {
  ...
  nh.param("/frontier_finder/min_candidate_occupied_clearance", min_candidate_occupied_clearance_, -1.0);
  ...
  if (resolution_ > 0.15) {
    cluster_min_ /= 2;
    min_visib_num_ *= 0.8;
    ROS_INFO("[FrontierFinder] Low resolution map detected, decrease cluster_min to %d, decrease min_visib_num to %d", cluster_min_, min_visib_num_);
  }
...
/**
 * @brief Search for frontiers in the updated region
*/
void FrontierFinder::searchFrontiers() {
  ...
  // Search new frontier within box
...
/**
 * @brief Expand the frontier cluster from the seed cell
*/
...
/**
 * @brief Sampling viewpoints around the frontier
*/
...
/**
 * @brief Sampling viewpoints around the frontier new
*/
void FrontierFinder::computeFrontiersToVisitNew(const vector<Vector3d> &all_centers_unknown_active) {
  first_new_ftr_ = frontiers_.end();
  int new_num = 0;
  int new_dormant_num = 0;
  // Try find viewpoints for each cluster and categorize them according to
  ...
  for (auto &tmp_ftr : tmp_frontiers_) {
    // Search viewpoints around frontier
    sampleViewpointsNew(tmp_ftr, all_centers_unknown_active);
    // sampleViewpoints3D(tmp_ftr);
    if (!tmp_ftr.viewpoints_.empty()) {
      ++new_num;
      list<Frontier>::iterator inserted = frontiers_.insert(frontiers_.end(), tmp_ftr);
...
void FrontierFinder::sampleViewpoints(Frontier &frontier) {
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_; rc <= candidate_rmax_ + 1e-3; rc += dr) {
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi) {
      ...
      if (!map_server_->isInBox(sample_pos) || map_server_->getOccupancy(sample_pos) == voxel_mapping::OccupancyType::OCCUPIED ||
          map_server_->getOccupancy(sample_pos) == voxel_mapping::OccupancyType::UNKNOWN || isNearUnknown(sample_pos))
        continue;
...
void FrontierFinder::sampleViewpointsNew(Frontier &frontier, const vector<Vector3d> & all_centers_unknown_active) {
  CHECK_EQ(frontier.viewpoints_.size(), 0) << "Frontier already has viewpoints";
  frontier.visib_num_ = 0;

  int effective_min_visib_num = min_visib_num_;
  bool is_near_unknown_center = false;

  for (const auto& center : all_centers_unknown_active) {
    if ((frontier.average_ - center).norm() < 5.0) {
      is_near_unknown_center = true;
      break;
    }
  }

  if (is_near_unknown_center) {
    effective_min_visib_num = min_visib_num_ / 2;
  }
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_; rc <= candidate_rmax_ + 1e-3; rc += dr) {
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi) {
      ...
      // 4. 使用新的动态阈值进行筛选
      if (visib_num > effective_min_visib_num) {
        Viewpoint vp = {sample_pos, avg_yaw, visib_num};
  ...
  double mean = 0.0;
  if (!visib_nums.empty()) {
    for (int num : visib_nums) {
      mean += num;
    }
    mean /= visib_nums.size();
  }
  double sd = 0.0;
  if (visib_nums.size() > 1) {
    for (int num : visib_nums) {
