...
PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh, int &drone_id) {
  node = nh;

  traj_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/trajectory_"+to_string(drone_id), 100);
  pubs_.push_back(traj_pub_);

  topo_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/topo_path_"+to_string(drone_id), 100);
  pubs_.push_back(topo_pub_);

  predict_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/prediction_"+to_string(drone_id), 100);
  pubs_.push_back(predict_pub_);

  visib_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/visib_constraint_"+to_string(drone_id), 100);
  pubs_.push_back(visib_pub_);

  frontier_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/frontier_"+to_string(drone_id), 1000);
  pubs_.push_back(frontier_pub_);

  yaw_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/yaw_"+to_string(drone_id), 100);
  pubs_.push_back(yaw_pub_);

  viewpoint_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/viewpoints_"+to_string(drone_id), 1000);
  pubs_.push_back(viewpoint_pub_);

  hgrid_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/hgrid_"+to_string(drone_id), 1000);
  pubs_.push_back(hgrid_pub_);

  debug_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/debug_"+to_string(drone_id), 1000);
  pubs_.push_back(debug_pub_);

  pose_pub_ = node.advertise<geometry_msgs::PoseStamped>("/planning_vis/pose_"+to_string(drone_id), 1000);
  pubs_.push_back(pose_pub_);

  pose_array_pub_ = node.advertise<geometry_msgs::PoseArray>("/planning_vis/pose_array_"+to_string(drone_id), 1000);
  ...
  trajectory_with_velocity_pub_ = node.advertise<sensor_msgs::PointCloud2>("/planning_vis/trajectory_with_velocity_"+to_string(drone_id), 1000);
...
void PlanningVisualization::drawYawPath(const vector<Eigen::Vector3d> &pos, const vector<double> &yaw) {
...
void PlanningVisualization::drawFrontierPointcloud(const vector<vector<Eigen::Vector3d>> &frontiers) {
...
void PlanningVisualization::drawTinyFrontierPointcloud(const vector<vector<Eigen::Vector3d>> &frontiers) {
...
void PlanningVisualization::drawDormantFrontierPointcloud(const vector<vector<Eigen::Vector3d>> &dormant_frontiers) {
...
void PlanningVisualization::drawFrontierPointcloudHighResolution(const vector<vector<Eigen::Vector3d>> &frontiers, const double &map_resolution) {
  if (frontier_pcl_pub_.getNumSubscribers() == 0)
    return;
...
void PlanningVisualization::drawTinyFrontierPointcloudHighResolution(const vector<vector<Eigen::Vector3d>> &frontiers, const double &map_resolution) {
  if (tiny_frontier_pcl_pub_.getNumSubscribers() == 0)
    return;
...
void PlanningVisualization::drawDormantFrontierPointcloudHighResolution(const vector<vector<Eigen::Vector3d>> &dormant_frontiers, const double &map_resolution) {
  if (dormant_frontier_pcl_pub_.getNumSubscribers() == 0)
    return;
...
// swarm 
void PlanningVisualization::drawAssignedGridCells(const vector<pair<Eigen::Vector3d, Eigen::Vector3d>>& grid_cells, 
                                                  int drone_id, const string& ns) {
  if (pubs_[ASSIGNED_GRIDS].getNumSubscribers() == 0)
    return;

  visualization_msgs::Marker cubes;
  // Set a base color based on drone_id and make it transparent
  Eigen::Vector4d color = getColor(static_cast<double>(drone_id) / 5.0, 0.3); // 5.0 is an arbitrary scaling factor

  // Use a fixed scale for all cubes in this list
  fillBasicInfo(cubes, Eigen::Vector3d(0,0,0), color, ns, drone_id, visualization_msgs::Marker::CUBE_LIST);
  cubes.action = visualization_msgs::Marker::ADD;

  for (const auto& cell_info : grid_cells) {
    geometry_msgs::Point center;
    center.x = cell_info.first.x();
    center.y = cell_info.first.y();
    center.z = cell_info.first.z();
    cubes.points.push_back(center);

    if (cubes.scale.x == 0) {
      cubes.scale.x = cell_info.second.x();
      cubes.scale.y = cell_info.second.y();
      cubes.scale.z = cell_info.second.z();
    }
  }
  pubs_[ASSIGNED_GRIDS].publish(cubes);
}
} // namespace fast_planner