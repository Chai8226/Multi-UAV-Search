...
const int BsplineOptimizer::SWARM = (1 << 7);
...
void BsplineOptimizer::setParam(ros::NodeHandle &nh) {
  ...
  nh.param("/bspline_opt/pos/swarm", pos_swarm_, -1.0);
  ...
  nh.param("/bspline_opt/swarm_safe_dist", swarm_safe_dist_, -1.0);
  ...
  if (false) {
    ...
    std::cout << "pos_time: " << pos_swarm_ << std::endl;
...
void BsplineOptimizer::combineCost(const std::vector<double> &x, std::vector<double> &grad, double &f_combine) {
  ...
  double ld_smooth, ld_dist, ld_feasi, ld_start, ld_end, ld_waypt, ld_time, ld_swarm;

  if (cost_function_ == (EXPLORATION_POSITION_PHASE | SWARM)) {
    ...
    ld_end = yaw_end_;
    ld_waypt = yaw_waypt_;
  } else if (cost_function_ == EXPLORATION_POSITION_PHASE) {
    ld_smooth = pos_smooth_;
    ld_dist = pos_dist_;
    ld_feasi = pos_feasi_;
    ld_start = pos_start_;
  ...
  if (cost_function_ & SWARM) {
    ...
    f_combine += ld_swarm * f_swarm;
    for (int i = 0; i < point_num_; i++)
      for (int j = 0; j < dim_; j++) grad[dim_ * i + j] += ld_swarm * g_swarm_[i](j);
  }
  ...
  if (false) {
    ...
    if (cost_function_ & SWARM)
      ROS_INFO("Swarm cost:            %f", ld_time * f_time);
  ...
  if (cost_function_ & SWARM)
    final_costs_.push_back(ld_swarm * f_swarm);
}
