...
public:
  ...
  PlanningVisualization(ros::NodeHandle &nh, int &drone_id);
  ...
  void drawSpheres(const vector<Eigen::Vector3d> &list, const double &scale, const Eigen::Vector4d &color, const string &ns, const int &id,
                   const int &pub_id);
  ...
  void drawTinyFrontierPointcloudHighResolution(const vector<vector<Eigen::Vector3d>> &frontiers, const double &map_resolution);

  // swarm
  void drawAssignedGridCells(const vector<pair<Eigen::Vector3d, Eigen::Vector3d>>& grid_cells, int drone_id, const string& ns);
