...
private:
  ...
  void calcSwarmCost(const vector<Eigen::Vector3d>& q, const double& dt, double& cost, vector<Eigen::Vector3d>& gradient);
  ...
  int order_;
  int bspline_degree_;
  double pos_smooth_, pos_dist_, pos_feasi_, pos_start_, pos_end_, pos_time_, pos_swarm_;
