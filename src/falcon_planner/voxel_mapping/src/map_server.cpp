...
MapServer::MapServer(ros::NodeHandle &nh, int &drone_id) {
  // swarm
  drone_id_ = drone_id;
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    nh.param("/map_config/map_size/box_min_" + axis[i], map_config.box_min_[i], map_config.map_min_[i]);
    nh.param("/map_config/map_size/box_max_" + axis[i], map_config.box_max_[i], map_config.map_max_[i]);
    nh.param("/map_config/map_size/vbox_min_" + axis[i], map_config.vbox_min_[i], map_config.map_min_[i]);
    nh.param("/map_config/map_size/vbox_max_" + axis[i], map_config.vbox_max_[i], map_config.map_max_[i]);
  }
  ...
  nh.param("/voxel_mapping/concurrent_pointcloud_input_max", config_.concurrent_pointcloud_input_max_, 1);
  ...
  nh.param("/voxel_mapping/occupancy_grid_slice_visualization_height", config_.occupancy_grid_slice_visualization_height_, -1.0);
  ...
  nh.param("/voxel_mapping/occupancy_grid/sdf_cutoff_distance_scale", sdf_cutoff_distance_scale, 1.0);
  ...
  tsdf_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/tsdf_"+to_string(drone_id_), 10);
  tsdf_slice_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/tsdf_slice_"+to_string(drone_id_), 10);
  esdf_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/esdf_"+to_string(drone_id_), 10);
  esdf_slice_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/esdf_slice_"+to_string(drone_id_), 10);
  occupancy_grid_occupied_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/occupancy_grid_occupied_"+to_string(drone_id_), 10);
  occupancy_grid_free_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/occupancy_grid_free_"+to_string(drone_id_), 10);
  occupancy_grid_unknown_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/occupancy_grid_unknown_"+to_string(drone_id_), 10);
  occupancy_grid_slice_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/occupancy_grid_slice_"+to_string(drone_id_), 10);
  interpolated_pose_pub_ = nh.advertise<nav_msgs::Odometry>("/voxel_mapping/interpolated_pose_"+to_string(drone_id_), 10);
  depth_pointcloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/voxel_mapping/depth_pointcloud_"+to_string(drone_id_), 10);
  map_coverage_pub_ = nh.advertise<std_msgs::Float32>("/voxel_mapping/map_coverage_"+to_string(drone_id_), 10);
  debug_visualization_pub_ = nh.advertise<visualization_msgs::Marker>("/voxel_mapping/debug_visualization_"+to_string(drone_id_), 10);

  // Initialize callbacks
  depth_sub_ = nh.subscribe("depth_image_topic", 1, &MapServer::depthCallback, this);
  // pointcloud_sub_ = nh.subscribe("/voxel_mapping/pointcloud", 1, &MapServer::pointcloudCallback, this);
  // global_map_sub_ = nh.subscribe("/voxel_mapping/global_map", 1, &MapServer::globalMapCallback, this);

  // Initialize service
  save_map_srv_ = nh.advertiseService("/voxel_mapping/save_map_pcd", &MapServer::saveMapPCDCallback, this);

  // Initialize timer
  publish_map_timer_ = nh.createTimer(ros::Duration(0.5), &MapServer::publishMapTimerCallback, this);
...
void MapServer::testESDFCallback(const geometry_msgs::PoseStamped &msg) {
  ...
  ROS_INFO("[MapServer] Test ESDF at position: %.2f, %.2f, %.2f, distance: %.2f, gradient: %.2f, "
           "%.2f, %.2f", pos(0), pos(1), pos(2), dist, grad(0), grad(1), grad(2));
}
...
// swarm add
void MapServer::setDroneId(const int &drone_id) {
  drone_id_ = drone_id;
}
