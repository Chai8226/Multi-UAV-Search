...
int main(int argc, char **argv) {
  ...
  nh.param("drone_id", drone_id_, -1);  // Initial position and yaw
  nh.param("init_x", init_pos[0], 0.0);
  nh.param("init_y", init_pos[1], 0.0);
  nh.param("init_z", init_pos[2], 0.0);
  nh.param("init_yaw", init_yaw, 0.0);
  ...
  ros::Subscriber bspline_sub = node.subscribe("planning/bspline_"+to_string(drone_id_), 1, bsplineCallback);
  ros::Subscriber replan_sub = node.subscribe("planning/replan_"+to_string(drone_id_), 1, replanCallback);

  pos_cmd_pub = node.advertise<quadrotor_msgs::PositionCommand>("planning/pos_cmd_"+to_string(drone_id_), 50);
  cmd_vis_pub = node.advertise<visualization_msgs::Marker>("planning/position_cmd_vis_"+to_string(drone_id_), 10);
  traj_pub = node.advertise<visualization_msgs::Marker>("planning/travel_traj_"+to_string(drone_id_), 10);
  traj_vel_pub = node.advertise<sensor_msgs::PointCloud2>("planning/travel_vel_traj_"+to_string(drone_id_), 10);
  ...
  double len = calcPathLength(traj_cmd_);
  double flight_t = (flight_end_time - flight_start_time).toSec();
  ROS_WARN("[TrajServer] Flight time: %.2lf, path length: %.2lf, mean vel: %.2lf", flight_t, len, len / flight_t);
