...
void FastPlannerManager::init(ros::NodeHandle &nh, int &drone_id) {
  // Load parameters
  ...
  map_server_.reset(new voxel_mapping::MapServer(nh, drone_id));
...
void FastPlannerManager::planExplorationPositionTraj(const vector<Eigen::Vector3d> &tour,
  ...
  if (trajs.empty()) {
    ROS_WARN("[FastPlannerManager] No swarm trajectories for collision avoidance.");
  } else {
